...
void solve()
{
   lli n,m,mx2=0,ans=0,i,j,sum=0,mn=100000000,l,k,mx=0,x=0,y=0,res,p,r,z=0,e;
   cin>>n>>m;
   ll a[n],q[m],s[m];
   vector<ll>vi,vd;
   vi.pb(0);
   for(i=0;i<n;i++)
   {
       cin>>a[i];
       if(i>0)x=a[i-1]-a[i];
       if(x>0)vi.pb(vi[i-1]+x);
       else if(i>0) vi.pb(vi[i-1]);
   }
   vd.pb(0);
   x=0;
   for(i=n-2;i>=0;i--)
   {
       ...
       x=a[i+1]-a[i];
       //cout<<x<<" ";
       if(x>0)vd.pb(vd[p-1]+x);
       else vd.pb(vd[p-1]);
   }
   reverse(vd);
   //cout<<endl;
   //for(i=0;i<n;i++)cout<<vd[i]<<" ";
  vector<ll>an;
   for(i=0;i<m;i++)
   {
       cin>>s[i]>>q[i];
       if(s[i]==q[i])an.pb(0);
       else if(s[i]<q[i])an.pb(vi[q[i]-1]-vi[s[i]-1]);
       else an.pb(vd[q[i]-1]-vd[s[i]-1]);
   }
   for(i=0;i<m;i++)cout<<an[i]<<endl;
